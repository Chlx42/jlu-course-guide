...
// 项目级覆盖文件（基于 themes/hextra @ 38d18a5, v0.12.3-17）。主题 submodule
// 更新后如需同步,请重新复制并重放下面标注「本站改动」的三处修改。
// 改动原因:中文按单字匹配时,「按章节标题命中数排序」会让大页面的常见字
// 片段霸榜;且页面标题不进索引,课程名只出现在标题里的课程页永远搜不到。
...
(function () {
  ...
  function preloadIndex() {
    ...
    const attempt = (async () => {
      ...
      for (const route in data) {
        // 本站改动: 页面标题计入页面索引(计两次权重,避免被罗列课程名的
        // 首页/导航页压过),课程名搜得到只写在标题里的课程页
        let pageContent = `${data[route].title} ${data[route].title}`;
        ++pageId;
        ...
        // 本站改动: 页面标题本身也登记为一条片段,保证页面级命中总能展示
        sectionIndex.add({
          id: route,
          url: route,
  ...
  function performSearch(query) {
    ...
    // 本站改动: 不再按「章节标题命中数」重排,直接沿用页面级相关度顺序,
    // 避免大页面的常见字片段把真正的课程页挤出前排。
    let optionId = 0;
    return groups
      .map((res) => ({
